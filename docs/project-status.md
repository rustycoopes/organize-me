...
**Slice 3 (LLM Prompt page) started.** Slice 2 fully drained. Issue #48 (Slice 3.0 — prompt foundation), the first Slice 3 issue, is implemented on branch `feature/slice-3.0-prompt-foundation` in an isolated worktree: a new `llm_prompts` table (one row per user, unique on `user_id`, `prompt_text` NOT NULL; migration `d3e4f5a6b7c8`), a single factory-default extraction-prompt constant (`app/core/prompts.py::FACTORY_DEFAULT_PROMPT`) shared by seeding and the later Reset button, and seed-on-registration via `UserManager.on_after_register` — one seam fastapi-users fires for both email/password and new-Google-user creation, and not on Google-link, so both paths seed exactly one prompt with no duplicate. Next: #49 (Slice 3.1 — prompt page + `/api/v1/llm-prompt` GET/PUT/reset), which is blocked by #48.
...
3. **Slice 3** — LLM Prompt page
   - #48 Prompt foundation (`llm_prompts` table + factory-default constant + seed on user creation) — 🔨 implemented on `feature/slice-3.0-prompt-foundation`
   - #49 Prompt page + `/api/v1/llm-prompt` GET/PUT/reset — ⏳ blocked by #48
