...
**Last updated:** 2026-07-16 (Issue #168 — Slice R13 Host Cleanup + "How to Add a Hosted App" Playbook)
...
| Date | Milestone |
|------|-----------|
| 2026-07-16 | Issue #168 (Slice R13 — Host Cleanup + "How to Add a Hosted App" Playbook) implemented on branch `restructure/r13-host-cleanup`. Removed the Host's now-dead event-extraction code (`event-creator` fully owns it, stable in prod since R12): page modules, API routers, `app/services/{pipeline,llm,storage}/`, the pipeline-only parts of `app/services/notifications/` (kept `email.py` for the Host's own password-reset emails), `app/core/{prompts,date_parser,calendar_url,message_filter,onboarding}.py`, `app/worker.py` (the never-wired Celery stub), their models/schemas/templates, and the now-unused `[program:worker]` block in `supervisord.conf`. Also stripped `notification_email`/`notification_sms` off `app/api/v1/users.py` (that data lived on the now-removed `event_creator.user_settings` model; R7 had already moved the Notifications Settings tab UI to `event-creator`). Deleted the matching Python/Playwright test files with verified-equivalent coverage in `event-creator`'s own suites (including `notifications.spec.ts`, found during review to have been missed from #168's original list — ported to `event-creator` in PR #17), and moved one Host-only DB-schema regression test (`test_host_users_no_longer_has_moved_columns`) into `tests/test_schema_separation.py`. Wrote `docs/platform-restructure/how-to-add-a-hosted-app.md`, the condensed playbook for a future app #3, validated against `event-creator`'s real app-registry entry/JWT-verify usage/chrome pin. Updated README + technical-approach for the Host-only surface and added this slice's section to `host-integration-guide.md`. `uv run mypy app tests` and `uv run pytest --collect-only` both clean after removal |
...
| 2026-07-15 | Issue #167 (Slice R12 — Production Cutover) in progress: provisioned the production LB fronting `organizeme.russcoopersoftware.com` (static IPs, DNS records, managed cert, NEGs, backend services, URL map, proxy, forwarding rules — new `provision-prod.{sh,ps1}` scripts, `generate_url_map.py` gained an env argument). Confirmed already-satisfied prerequisites: prod Cloud Tasks queue provisioned, both prod services on request-based billing, `E2E_TEST_MODE` unset on both |
| 2026-07-15 | Issue #167 continued: cert went `ACTIVE` (faster than the ~24h estimate); routing verified (`/login` → Host, `/dashboard` → Event Creator). User registered the new redirect URIs on the Google OAuth client in Cloud Console and confirmed `google-oauth-client-secret-prod` matches the real client secret. Flipped `GOOGLE_OAUTH_REDIRECT_URI`/`GOOGLE_DRIVE_REDIRECT_URI` in `organize-me`'s `deploy.yml` to the LB domain (`event-creator`'s was already pointed there since #200) |
| 2026-07-15 | Issue #167 done: post-cutover smoke testing surfaced and fixed two live bugs — a stale Google Drive OAuth token in prod (encrypted before `ENCRYPTION_KEY` moved to Secret Manager; fixed by reconnecting Drive, no code change) and a missing `GEMINI_API_KEY` in `event-creator` (never wired in since R11 moved the pipeline there; fixed in `event-creator` PR #14). Real login and a real upload both confirmed working end-to-end in prod. Slice R12 (Production Cutover) complete |
