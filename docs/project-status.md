...
**Slice 2 (Google Drive storage) in progress.** Slice 1 is fully drained — issue #23 (Playwright E2E) merged (PR #57) with its prod deploy green. Issue #45 (Slice 2.0 — storage foundation) merged (PR #58): the `storage_configs` table + migration, the `StorageProvider` ABC + `FakeStorageProvider`, and Fernet-based credential-encryption helpers (`app/core/security.py`). Issue #46 (Slice 2.1 — Settings > Storage tab + storage-config read/write) is implemented on branch `feature/slice-2-storage-tab`: `GET`/`PUT /api/v1/storage-config` (credential-safe read, single-row upsert) behind a new Settings > Storage page with an Alpine.js provider dropdown that conditionally shows the Google Drive fields and hides the Dropbox/S3 stubs, plus a Playwright spec in the `e2e-qa` job. No live OAuth yet — that's #47. New **human setup** before the credential-write paths go live: create an `ENCRYPTION_KEY` GitHub secret (a `Fernet.generate_key()` value) — it's already wired into `ci.yml`/`deploy.yml` and defaults empty until then.
...
2. **Slice 2** — Google Drive storage integration
   ...
   - #46 Settings > Storage tab + storage-config read/write — 🔨 implemented on `feature/slice-2-storage-tab` (PR open)
   - #47 Google Drive OAuth connect/disconnect + onboarding flag — next
