...
**Last updated:** 2026-07-15 (Issue #167 in progress — Slice R12 production LB provisioned)
...
| 2026-07-15 | Issue #167 (Slice R12 — Production Cutover) in progress: provisioned the production LB fronting `organizeme.russcoopersoftware.com` (static IPs, DNS records, managed cert, NEGs, backend services, URL map, proxy, forwarding rules — new `provision-prod.{sh,ps1}` scripts, `generate_url_map.py` gained an env argument). Non-disruptive — no existing traffic redirected yet. Confirmed already-satisfied prerequisites: prod Cloud Tasks queue provisioned, both prod services on request-based billing, `E2E_TEST_MODE` unset on both. Remaining: cert to go `ACTIVE` (~24h), register new redirect URIs on the Google OAuth client in Cloud Console, verify `google-oauth-client-secret-prod`, flip `GOOGLE_OAUTH_REDIRECT_URI`/`GOOGLE_DRIVE_REDIRECT_URI` to the LB domain, post-cutover smoke tests |
