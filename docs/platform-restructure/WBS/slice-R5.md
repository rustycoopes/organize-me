...
**Delivers:** A GCP External HTTPS Load Balancer fronting `organizeme.qa.russcoopersoftware.com`
(QA), with a path-based URL map routing requests to the correct Cloud Run service and a
Google-managed SSL certificate — the shared single origin the whole platform sits behind. (The
production origin `organizeme.russcoopersoftware.com` is provisioned the same way at cutover, R12.)
...
service, a Google-managed cert for `organizeme.qa.russcoopersoftware.com`, and a URL map whose path
rules come from the app-registry authored in R3. At this slice there is still only the Host service
to route to (Event Creator arrives in R6); the LB is stood up so the boundary tracer bullet in R6
has a shared origin.

## Includes
- External HTTPS Load Balancer + global static IP.
- Google-managed SSL certificate for `organizeme.qa.russcoopersoftware.com`; a **Custom A/AAAA
  record** in Squarespace DNS pointed at the LB's global static IP (uses R0's DNS access). Use
  Custom Records — **not** Squarespace Domain Forwarding, which is an HTTP redirect and would block
  cert validation and the shared-origin cookie.
...
- Managed cert issuance requires the A/AAAA record to resolve to the LB — sequence the Squarespace
  Custom Record before cert validation.
- Keep `*.run.app` URLs alive in parallel until the QA/prod cutovers (R11/R12) flip the origin.

## Blocked by
- R0 (editable DNS for the `russcoopersoftware.com` subdomains).

## Acceptance criteria
- [ ] An External HTTPS LB with a global IP and a **valid** Google-managed cert serves
      `https://organizeme.qa.russcoopersoftware.com` (QA setup).
...
## Testing
- `curl https://organizeme.qa.russcoopersoftware.com/login` returns the Host login page over a valid TLS cert.
