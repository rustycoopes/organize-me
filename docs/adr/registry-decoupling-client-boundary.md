...
- Each consumer also carries a duplicated *data* copy, not just a duplicated loop: organize-me's
  own `app/core/registry.py` fork of `APPS` and each fetch-based consumer's own cold-start
  `AppEntry` (e.g. event-creator's `SELF_APP_ENTRY`) must be kept in sync by hand — accepted for
  the same reason as the scheduling-loop duplication above (each consumer needs to be able to vouch
  for its own surface without a runtime dependency on anything else). Registry-decoupling Slice 3
  (organize-me#220) removed the third copy that used to exist alongside these two —
  `organizeme_chrome.registry`'s compiled-in fallback literal — once all three consumers were
  confirmed running their own real `RegistrySource`.
