...
### Added
- 2026-09-10 — event-creator#50 mobile-responsive-tables Slice 2: Event Creator Logs page adopts `.om-stacked-table` (runs grid → labelled cards below `lg`) and the filter/sort form collapses behind a pure-CSS `Filters (N)` disclosure on mobile, with `sort_by`/`sort_dir` `<select>`s added for mobile sorting (the grid's `<thead>` sort links are card-hidden). Rode along: `event-creator`'s QA pipeline dropped (mirrors `organize-me`'s 2026-09-08 change; CI `test` job now runs against Supabase prod, Playwright `e2e/` is local-only) — [details](features/mobile-responsive-tables/WBS/slice-2-logs.md#delivered-2026-09-10-issue-event-creator50-branch-featuremobile-responsive-tables-logs)
- 2026-09-08 — #276 mobile-responsive-tables Slice 1a: shared `.om-stacked-table` responsive pattern shipped from `packages/chrome` (`chrome-v0.20.1`) — `components.css` + `chrome_components_css_path()` + `STACKED_TABLE_CLASS`; Host imports it inert (no visual change) — [details](features/mobile-responsive-tables/WBS/slice-1-chrome-pattern-and-dashboard.md#delivered--slice-1a-2026-09-08-issue-276-branch-featuremobile-responsive-tables-chrome-pattern)
- 2026-09-08 — event-creator#49 mobile-responsive-tables Slice 1b: Event Creator Dashboard adopts `.om-stacked-table` (events table → labelled cards below `lg`, `chrome-v0.20.1` pin + `components.css` import + `verify_css_build` canary) and the filter/sort form collapses behind a pure-CSS `Filters (N)` disclosure on mobile — [details](features/mobile-responsive-tables/WBS/slice-1-chrome-pattern-and-dashboard.md#delivered--slice-1b-2026-09-08-issue-event-creator49-branch-featuremobile-responsive-tables-dashboard)
- 2026-07-28 — event-creator#42 Dashboard bulk actions Slice 2: Bulk mark as reviewed — `POST /api/v1/events/bulk-review` (best-effort, idempotent on already-reviewed events, no `WHERE reviewed IS FALSE` guard), "Mark Selected as Reviewed" (primary/cobalt) toolbar button applying with no confirmation dialog — [details](https://github.com/rustycoopes/event-creator/blob/main/docs/features/dashboard-bulk-actions/WBS/slice-2-bulk-mark-reviewed.md#delivered-2026-07-28-issue-42-branch-featuredashboard-bulk-mark-reviewed)
- 2026-07-27 — ha-dashboard#17 static-asset-routing Slice 6 follow-up: `no_db_client` test fixture (plain `ASGITransport`, no DB dependency) so static-mount and health-check tests run without a reachable Postgres — [details](features/static-asset-routing/WBS/slice-6-ha-dashboard-migration.md#follow-up-delivered-2026-07-27-ha-dashboard17-branch-fixstatic-mount-tests-no-db-in-ha-dashboard)
- 2026-07-27 — event-creator#41 Dashboard bulk actions Slice 1: Bulk delete — `POST /api/v1/events/bulk-delete` (best-effort, `succeeded_ids`/`failed_ids`), checkbox column with select-all/indeterminate state, bulk-action toolbar, and confirm dialog on the dashboard events table — [details](https://github.com/rustycoopes/event-creator/blob/main/docs/features/dashboard-bulk-actions/WBS/slice-1-bulk-delete.md#delivered-2026-07-27-issue-41-branch-featuredashboard-bulk-delete)
- 2026-07-27 — event-creator#37 local-dev-environment Slice 5: `mock_integrations` flag — `Settings.mock_integrations` OR'd into `build_storage_provider()`/`get_gemini_client()`/`get_import_storage()`; `get_pipeline_notifier()` gains its first real/fake branch (`RealNotificationSender` with fake email/SMS senders injected) — [details](features/local-dev-environment/WBS/slice-5-event-creator-mock-integrations.md#delivered-2026-07-27-issue-event-creator37-branch-featuremock-integrations-flag)
- 2026-07-26 — ha-dashboard#18 local-dev-environment Slice 7: `ha-dashboard` local-dev launcher integration — `scripts/dev.py`, consumer-side `registry_local_dev_bypass` wiring, README pointer; companion `organizeme_chrome` changes in #273/#274 (`chrome-v0.19.1`) — [details](features/local-dev-environment/WBS/slice-7-ha-dashboard-launcher-integration.md#delivered-2026-07-26-issue-ha-dashboard18-branch-featurelocal-dev-launcher-integration)
- 2026-07-26 — event-creator#36 local-dev-environment Slice 4: `event-creator` local-dev launcher integration — `scripts/dev.py`, consumer-side `registry_local_dev_bypass` wiring, `organizeme-chrome` pin bump to v0.19.0, README pointer — [details](features/local-dev-environment/WBS/slice-4-event-creator-launcher-integration.md#delivered-2026-07-26-issue-event-creator36-branch-featurelocal-dev-launcher-integration)
...
- 2026-07-26 — ha-dashboard#19 local-dev-environment Slice 8: `mock_integrations` flag — `Settings.mock_integrations`, `build_ha_transport_factory`, `FakeHATransport` promoted to application code — [details](features/local-dev-environment/WBS/slice-8-ha-dashboard-mock-integrations.md#delivered-2026-07-26-issue-rustycoopesha-dashboard19-branch-featureslice-8-mock-integrations)
...
### Fixed
- 2026-07-28 — #201 Dropbox OAuth connect had the same `redirect_uri_mismatch` bug as #200: `event-creator`'s `_dropbox_redirect_uri(request)` built the callback URL from the incoming request's Host header rather than a fixed setting. Added `DROPBOX_OAUTH_REDIRECT_URI`, wired into `app/core/config.py`/`.env.local.example`/`ci.yml`/`deploy.yml` and a fail-fast check in `/auth`, mirroring #200's fix exactly — [details](https://github.com/rustycoopes/event-creator/pull/45)
- 2026-07-25 — #257 Fix ha-dashboard/QA registry-backend mismatch: `generate_url_map.py` emitted a QA path rule for `ha-dashboard`, which deliberately has no QA Cloud Run service/backend — re-running QA's URL-map generation would have pointed at a nonexistent backend service and been rejected by `gcloud compute url-maps import`. Static-asset-routing feature prerequisite — [details](features/static-asset-routing/WBS/slice-1-ha-dashboard-qa-registry-fix.md#delivered-2026-07-25-issue-257-branch-fixha-dashboard-qa-registry-mismatch)
- 2026-07-23 — #249 Settings shell hardcoded every tab's HTMX fragment fetch to event-creator (R7 leftover) — never actually reachable for a second app's tab despite the registry listing it; now aggregates settings_tabs across every registered app and routes each to its own owning service_name — discovered while implementing ha-dashboard#3 — [details](https://github.com/rustycoopes/organize-me/pull/249)
