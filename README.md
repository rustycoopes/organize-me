...
**In development** — Slice 1 (project scaffold, auth, CI/CD) and Slice 2 (Google Drive storage) complete: the storage foundation, the Settings > Storage tab (`GET`/`PUT /api/v1/storage-config`), and the Google Drive OAuth connect/disconnect flow (tokens encrypted at rest) are all in. Slice 3 (LLM Prompt page) complete: an `llm_prompts` table with a factory-default extraction prompt seeded into every new account, plus a Prompt page (`/prompt`) and `GET`/`PUT /api/v1/llm-prompt` + `POST /api/v1/llm-prompt/reset` endpoints letting a user view, edit, and reset that prompt. Slice 4 (upload + processing pipeline) in progress: the pipeline foundation is in — the `processing_runs`/`processing_steps`/`events` tables (with duplicate-detection), a `resolved_date` → earliest-date parser, and an injectable Gemini call wrapper (fake in tests) — and now the **Upload page + `POST /api/v1/upload` + the 7-step pipeline** (in-process asyncio, not Celery): an uploaded `.txt`/`.zip`/`.csv` is written to the connected Drive folder, run through the pipeline, and its events land in the DB, with per-step progress recorded — and now a **live SSE progress page** (`/processing` + `GET /api/v1/processing-runs/{id}/sse`) streams those 7 steps to the browser (HTMX SSE) as the run advances, with clear success/failure terminal states — completing Slice 4. Slice 5 (events dashboard) started: the **`/dashboard` events table** (`GET /api/v1/events`, 50/page, newest first) now renders each extracted event with `agreed_by` initials chips, per-row pre-filled **Google Calendar / Google Task** links (`app/core/calendar_url.py`), and a modal-gated **delete** (`DELETE /api/v1/events/{id}`); filters/sort/search follow in #55. See [Project Status](docs/project-status.md). **Human setup** to run things live: register the Drive OAuth callback redirect URI + `drive` scope on the Google client; set the `ENCRYPTION_KEY` and `GEMINI_API_KEY` secrets; enable Cloud Run "CPU always allocated" so the background pipeline task survives past the HTTP response; and verify `GoogleDriveStorageProvider` against a real connected Drive account (its live behaviour isn't exercised by CI).
